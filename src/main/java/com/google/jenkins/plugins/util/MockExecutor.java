...
/**
 * This is an implementation of {@link Executor} that can be injected
 * to inject a set of canned responses to requests including:
 * <ul>
...
public class MockExecutor extends Executor {
   ...
   */
  @Override
  public <T> T execute(RequestCallable<T> request)
  ...
  public <T> T execute(AbstractGoogleJsonClientRequest<T> request)
    ...
    Predicate<AbstractGoogleJsonClientRequest<T>> predicate =
        (Predicate<AbstractGoogleJsonClientRequest<T>>)
        predicates.removeFirst();

    if (requestClass != clazz) {
      sawUnexpected = true;
      throw new IllegalStateException("Unexpected (or out of order) request: " +
          requestClass + " expected: " + clazz);
    }

    if (!predicate.apply(request)) {
      sawUnexpected = true;
      throw new IllegalStateException("User predicate: " + predicate +
          " failed for request: " + requestClass);
    }
  ...
  /**
   * When the next request matches the given {@code requestType} and the
   * provided user {@link Predicate} return {@code response} as the response.
   */
  public <T, S extends AbstractGoogleJsonClientRequest<T>, C extends S>
      void when(
  ...
  /**
   * When the next request matches the given {@code requestType} return
   * {@code response} as the response.
   */
  ...
  /**
   * When the next request matches the given {@code requestType} and the
   * provided user {@link Predicate} throw {@code exception} instead of
   * responding.
   */
  public <T, S extends AbstractGoogleJsonClientRequest<T>, C extends S>
      void throwWhen(
  ...
  /**
   * When the next request matches the given {@code requestType} throw
   * {@code exception} instead of responding.
   */
   ...
   * responding.
   */
  private <T, S extends AbstractGoogleJsonClientRequest<T>, C extends S>
                void throwWhenInternal(
                    Class<C> requestType, Exception exception,
                    Predicate<S> predicate) {
    requestTypes.add(checkNotNull(requestType));
    responses.add(null);
  ...
  /**
   * When the next request matches the given {@code requestType} and the
   * provided user {@link Predicate} pass through the request's
   * {@code getJsonContent()} cast to the expected response type.
   */
  public <T, S extends AbstractGoogleJsonClientRequest<T>, C extends S>
      void passThruWhen(
  ...
  /**
   * When the next request matches the given {@code requestType} pass through
   * the request's {@code getJsonContent()} cast to the expected response type.
   */
  public <T, C extends AbstractGoogleJsonClientRequest<T>> void passThruWhen(
      Class<C> requestType) {
